#!/bin/sh
#
# PWRMON v1.16 - Asus-Merlin Tesla Powerwall Monitor by Viktor Jaep, 2022
#
...
DisplayPage1 () {
  ...
  echo -e "                         ${InvWhite}${CBlack}    $battery kW      ${InvDkGray}${CGreen}|${CClear}${CGreen} CHARGE:${CWhite} $battcapp %${CClear}"
  echo -e "                         ${InvWhite}                  ${InvDkGray}${CGreen}|${CClear}"
  echo -e "                         ${InvWhite}${CRed}    T E S L A     ${InvDkGray}${CGreen}|${CClear}"
  echo -e "                         ${InvWhite}                  ${InvDkGray}${CGreen}|${CClear}"
  if [ "$remaining" != "0" ]; then
  echo -e "                         ${InvWhite}${CRed}  $remaining/h runtime ${InvDkGray}${CGreen}|${CClear}${CGreen} PW MODE:${CWhite} $realmode${CClear}"
  else
  echo -e "                         ${InvWhite}                  ${InvDkGray}${CGreen}|${CClear}${CGreen} PW MODE:${CWhite} $realmode${CClear}"
  fi
  echo -e "                         ${InvWhite}                  ${InvDkGray}${CGreen}|${CClear}${CGreen} RESERVE:${CWhite} $backupreserve %${CClear}"
...
while true; do
  ...
  if [ "$gridstatus" == "SystemIslandedActive" ]; then
    ...
    echo -e "$(date) - PWRMON ---------> GRID ONLINE - Powerwalls and Gateway are Syncing!" >> $LOGFILE
    griddown=1
  fi

  if [ "$gridstatus" != "SystemIslandedActive" ] && [ "$gridstatus" != "SystemTransitionToGrid" ] && [ "$griddown" == "1" ]; then
    echo -e "$(date) - PWRMON ---------> GRID ONLINE - Powerwalls and Gateway returning to normal operation at $battcapp% remaining!" >> $LOGFILE
    griddown=0
  fi
