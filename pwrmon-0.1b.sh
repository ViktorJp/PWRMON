...
# electical grid outage, PWRMON will calculate your estimated remaining runtime left on your batteries based on the
...
# would compiment other dashboard-like scripts greatly (like RTRMON or VPNMON-R2), sitting side-by-side in their own SSH
...
logo () {
  ...
  echo -e "   / ____/| |/ |/ / _, _/ /  / / /_/ / /|  /   ${CClear}(N)${CGreen}ext Page ($NextPage/1)${CYellow}"
    ...
    while true; do
      ...
      # Execute chosen selections
          case "$ConfigSelection" in
            ...
            5) # -----------------------------------------------------------------------------------------
              echo ""
              echo -e "${CCyan}4. How many Tesla Powerwalls do you currently have configured?"
...
sitename="My Tesla Site Name"
  ...
  echo -e "${CGreen}/${CRed}Tesla Powerwall${CClear}${CGreen}\____________________________________________________${CClear}"
  echo -e "                                                                      "
  ...
  echo -e "${CYellow}                                 $solarshowline                          ${CClear}"
  echo -e "${CYellow}                                 $solarshowline                          ${CClear}"
  echo -e "         ${CWhite}${InvDkGray} _/====\_ ${CClear}             ${CYellow}$solardirection1           ${CWhite}  ${InvBlue}  /====\| ${CClear}"
  echo -e "         ${CWhite}${InvDkGray} ! \||/ ! ${CClear}             ${CYellow}$solardirection2           ${CWhite}  ${InvBlue} /______\ ${CClear}"
  echo -e "         ${CWhite}${InvDkGray}   _||_   ${CClear}$gridshowlines${CBlack}${InvWhite}[GW]${CClear}$homeshowlines${CWhite}${InvBlue} | |[]| | ${CClear}"
  echo -e "         ${CWhite}${InvDkGray} ^^^^^^^^ ${CClear}             ${CGreen}$batterydirection1          ${CWhite}   ${InvBlue} ^^^^^^^^ ${CClear}"
  echo -e "          $grid kW              ${CGreen}$batterydirection2           ${CBlue}   $home kW${CClear}"
  echo -e "         ${CWhite}${InvDkGray}   GRID   ${CClear}             ${CGreen}$batterydirection3         ${CBlue}    ${CWhite}${InvBlue}   HOME   ${CClear}"
  ...
  echo -e "                         ${InvWhite}                  ${InvDkGray}${CGreen}|${CClear}"
  if [ "$remaining" != "0" ]; then
  echo -e "                         ${InvWhite}${CRed} $remaining/h remaining ${InvDkGray}${CGreen}|${CClear}"
  else
  echo -e "                         ${InvWhite}                  ${InvDkGray}${CGreen}|${CClear}"
  fi
  echo -e "                         ${InvWhite}                  ${InvDkGray}${CGreen}|${CClear}"
  echo -e "                         ${InvDkGray}${CWhite}  $numpowerwalls POWERWALL(S)   ${CClear}"
  echo -e "${CGreen}_____________________________________________________________________${CClear}"
}
...
while true; do
  ...
  if [ "$batteryoutput" -ge "100" ]; then
    ...
    remaining=$(awk -v battload=$battery -v pwalls=$numpowerwalls -v pwcap=13.5 -v remain=$battmult 'BEGIN{printf "%+04.f\n", pwalls*pwcap*remain/battload}')
