  ...
  if [ -f $CFGPATH ]; then #Making sure file exists before proceeding
    ...
    while true; do
      ...
      if [ "$emailalerts" == "Enabled" ]; then
        ...
        echo -e "${InvDkGray}${CWhite} |-${CClear}${CCyan}-  SMS/Text Address RCPT TO:  :"${CGreen}$smsrecipient
        ...
        echo -e "${InvDkGray}${CWhite} | ${CClear}${CDkGray}-  SMS/Text Address RCPT TO:  :"${CDkGray}$smsrecipient
      ...
      echo -en "${InvDkGray}${CWhite} 9 ${CClear}${CCyan}: Progress Bar Preference?    :"${CGreen}
      if [ "$ProgPref" == "0" ]; then
      ...
      # Execute chosen selections
          case "$ConfigSelection" in
            ...
            8) # -----------------------------------------------------------------------------------------
              ...
              echo ""
              echo -e "${CYellow}NOTE: SMS/Text Address needs to be in the format:"
              echo -e "${CYellow}1234567890@carrierdomainname.com - please see instructions"
              echo -e "${CYellow}for further info or compatibility with your carrier."
              echo ""
              echo -e "${CYellow}(Email Notification Default = No)${CClear}"
              if promptyn "Enable Notifications? (y/n): "; then
                ...
                echo ""
                read -p 'Enter RCPT TO SMS/Text Address (Type 'Disabled' for none): ' smsrecipient1
                if [ ! -z "$smsrecipient1" ]; then smsrecipient=$smsrecipient1; fi
                echo -e "${CGreen}Using: $smsrecipient${CClear}"
                echo ""
                ...
                echo ""
                echo -e "${CCyan}Would you like to receive a test notification?${CClear}"
                if promptyn "Test email/sms notification settings? (y/n): "; then
                  emailalert test
                  smsalert test
                  echo ""
                  echo -e "${CGreen}Notification(s) sent.  Please check your inbox/junk mail folder!"
                  sleep 3
                fi
              else
                emailalerts="Disabled"
            ...
            [Ss]) # -----------------------------------------------------------------------------------------
              ...
              { echo 'Interval='$Interval
                ...
                echo 'smsrecipient="'"$smsrecipient"'"'
              ...
              echo ""
              echo -e "${CGreen}Applying config changes to PWRMON..."
...
vupdate () {
  ...
  if [ "$Version" == "$DLVersion" ]
    then
      ...
      if promptyn "(y/n): "; then
        echo ""
        echo -e "${CCyan}Downloading PWRMON ${CYellow}v$DLVersion${CClear}"
...
vsetup () {
  ...
  if ! grep -F "sh /jffs/scripts/pwrmon.sh" /jffs/configs/profile.add >/dev/null 2>/dev/null; then
...
DisplayPage2 () {
  ...
  runtimenum=$(awk -v homeload=$home -v pwalls=$numpowerwalls -v pwcap=13.5 -v remain=$battmult 'BEGIN{printf "%.0f\n", pwalls*pwcap*remain/homeload}')
  if [ "$runtimenum" -ge "100" ]; then runtime="+99.9"; fi

  echo -e "${InvCyan} ${CClear} ${CCyan}Runtime      ${CGreen}[            ${CCyan}$runtime/h${CGreen}               ] ${InvDkGray}${CWhite}(Home Load)${CClear}"
...
while true; do
  ...
  if [ "$batteryoutput" -ge "100" ]; then
    ...
    remainingnum=$(awk -v battload=$battery -v pwalls=$numpowerwalls -v pwcap=13.5 -v remain=$battmult 'BEGIN{printf "%.0f\n", pwalls*pwcap*remain/battload}')
    if [ "$remainingnum" -ge "100" ]; then remaining="+99.9"; fi
  ...
  if [ "$gridstatus" != "SystemIslandedActive" ] && [ "$gridstatus" != "SystemTransitionToGrid" ] && [ "$griddown" == "1" ]; then
    echo -e "$(date) - PWRMON ---------> GRID BACK ONLINE - Powerwalls and Gateway returning to normal operations at $battcapp% remaining!" >> $LOGFILE
    griddown=0
    emailalert normal
    smsalert normal
  fi
  ...
  while [ $i -ne $Interval ]
    do
      ...
      if [ $key_press ]; then
          case $key_press in
              [Ss]) FromUI=1; (vsetup); source $CFGPATH; echo -e "${CGreen}[Returning to the Main UI momentarily]                                   "; sleep 1; FromUI=0; clear; DisplayPage$NextPage;;
              ...
              [Ww]) (weathercheck);;
              [Rr]) if [ "$autorotate" == 0 ]; then autorotate=1; autorotateindicator="ON"; clear; DisplayPage$NextPage; elif [ "$autorotate" == "1" ]; then autorotate=0; autorotateindicator="OFF"; clear; DisplayPage$NextPage; fi;;
      ...
      if [ "$autorotate" == "1" ] && [ $Interval -eq $i ]; then
        ...
        elif [ "$NextPage" == "3" ]; then clear; NextPage=1 #DisplayPage1
        fi
      fi
  done
