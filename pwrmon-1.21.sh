#!/bin/sh
#
# PWRMON v1.21 - Asus-Merlin Tesla Powerwall Monitor by Viktor Jaep, 2022
#
  ...
  then
    ...
    while [ $i -ne 6 ]
      do
        ...
        WANwxName=$(cat $WANwxforecast | jq -r '.properties.periods['$i'].name // null')
        WANwxTemp=$(cat $WANwxforecast | jq -r '.properties.periods['$i'].temperature // null')
        WANwxTempUnit=$(cat $WANwxforecast | jq -r '.properties.periods['$i'].temperatureUnit // null')
        WANwxWind=$(cat $WANwxforecast | jq -r '.properties.periods['$i'].windSpeed // null')
        WANwxWindDir=$(cat $WANwxforecast | jq -r '.properties.periods['$i'].windDirection // null')
        WANwxShort=$(cat $WANwxforecast | jq -r '.properties.periods['$i'].shortForecast // null')
        WANwxDetail=$(cat $WANwxforecast | jq -r '.properties.periods['$i'].detailedForecast // null')
